...
| `{% imgtext pic1.jpg 60% 这里是注释 %}` | 图片居中，注释显示在图片下方（字号比正文小一号） |
| `{% imgtext pic1.jpg 300px 注释 %}` | 宽度用像素，注释同样在图片下方 |

宽度支持 `%`、`px`、`em`、`rem`、`vw`，只写数字按百分比处理。注释里有空格时用引号包起来：`{% imgtext pic1.jpg 60% "带 空格 的注释" %}`。
